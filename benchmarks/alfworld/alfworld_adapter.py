...
class AlfworldAdapter:
    ...
    def _load_config(self, config_path: Optional[str]) -> Dict[str, Any]:
        ...
        if config_path and os.path.exists(config_path):
            ...
            config = self._expand_config_paths(config)
    ...
    def _expand_config_paths(self, config: Dict[str, Any]) -> Dict[str, Any]:
        """Expand environment variables and user paths in config."""
    ...
    def _validate_config_paths(self, config: Dict[str, Any]) -> None:
        """Validate critical ALFWorld dataset/config paths exist."""
        dataset = config.get("dataset", {})
        logic = config.get("logic", {})

        required_paths = [
            dataset.get("data_path"),
            dataset.get("eval_id_data_path"),
            dataset.get("eval_ood_data_path"),
            logic.get("domain"),
            logic.get("grammar"),
        ]

        missing = [path for path in required_paths if path and not os.path.exists(path)]
        if missing:
            missing_list = "\n".join(f"- {path}" for path in missing)
            raise FileNotFoundError(
                "ALFWorld config paths missing:\n"
                f"{missing_list}\n"
                "Set ALFWORLD_DATA or update benchmarks/alfworld/alfworld_config.yaml"
            )

    @staticmethod
    def check_environment(config_path: Optional[str] = None) -> Dict[str, Any]:
        """
        Comprehensive environment self-check for fast failure.

        Validates:
        1. ALFWorld package importable
        2. Config file loadable and paths valid
        3. Environment can be created and reset

        Returns:
            Dict with 'ok' bool and optional 'error'/'warnings' fields.
        """
        warnings: List[str] = []
        ...
        try:
            ...
            return {"ok": False, "error": f"ALFWorld package not importable: {e}"}

        try:
            adapter = AlfworldAdapter.__new__(AlfworldAdapter)
            config = adapter._load_config(config_path)
        except FileNotFoundError as e:
            return {"ok": False, "error": f"Config/dataset paths missing: {e}"}
        except Exception as e:
            return {"ok": False, "error": f"Config load failed: {e}"}

        data_path = config.get("dataset", {}).get("data_path", "")
        if data_path and not os.path.exists(
            os.path.expanduser(os.path.expandvars(data_path))
        ):
    ...
    def _create_environment(self):
        ...
        try:
            env = alf_env.get_environment(env_type)(
                self.config, train_eval=self.train_eval
            )
            return env
        except Exception as e:
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "logic": {"domain": "logic/alfred.pddl", "grammar": "logic/alfred.twl2"},
            "env": {
                "type": "AlfredTWEnv",
