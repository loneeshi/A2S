...
class ReflectionTrace:
    ...
    Structured reflection trace aligned with core.reflection.schema.ReflectionOutput.
    All analysis fields are typed — no free-text reflection_content.
    """
    ...
    # Structured analysis (aligned with ReflectionOutput)
    failure_type: str = ""
    root_cause: str = ""
    ...
    # Legacy field kept for backward compatibility — prefer structured fields above
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "prompt_update_action": self.prompt_update_action,
            "prompt_update_content": self.prompt_update_content,
...
class DynamicMemory:
    ...
    Captures agent memory state aligned with core.memory module structure.
    """
    ...
    # Structured fields aligned with MemoryManager
...
class TraceEvent:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "llm_call_trace": self.llm_call_trace.to_dict()
            if self.llm_call_trace
            else None,
            "tool_call_record": self.tool_call_record.to_dict()
            if self.tool_call_record
            else None,
            "handoff_vector": self.handoff_vector.to_dict()
            if self.handoff_vector
            else None,
            "reflection_trace": self.reflection_trace.to_dict()
            if self.reflection_trace
            else None,
...
def create_task_trace(task_name: str, task_description: str, **kwargs) -> TaskTrace:
    """创建任务追踪的工厂函数"""
    return TaskTrace(
