...
export class BenchmarkBridge {
  ...
  private async startServer(): Promise<void> {
    ...
    const serverScript = join(projectRoot, "system", "bridge", "bench_server.py")
    ...
    if (this.options.stratifiedPerType && this.options.stratifiedPerType > 0) {
      args.push("--stratified-per-type", String(this.options.stratifiedPerType))
    }
